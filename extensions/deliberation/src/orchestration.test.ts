...
function requestFor(recorded: RecordedRequest[], path: string): RecordedRequest {
  ...
  return matching[0] as RecordedRequest;
}
...
describe("Deliberation cross-provider orchestration", () => {
  it.each([
    ...
    async ({ providerEventId, threadId, expectedHistoryIds }) => {
      ...
      await withServer(createKmFake(recorded), async (kmEndpoint) => {
        const api = createTestPluginApi({
          ...
          registerGatewayMethod: ((name: string, handler: (...args: never[]) => Promise<void>) => {
            ...
            channel: {
              outbound: {
                loadAdapter: vi.fn(async (provider: string) => ({
                  sendText: provider === "discord" ? discordSendText : slackSendText,
                  textChunkLimit: provider === "discord" ? 2_000 : 4_000,
      ...
      expect(requestFor(recorded, "/deliberation/v1/reservations").body).toMatchObject({
        deliveryTarget: wireTarget,
      });
