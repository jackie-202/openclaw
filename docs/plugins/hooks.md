...
- **`inbound_claim`** - claim an inbound message before agent routing (synthetic replies). Its event includes the host-authoritative `provider`, the fixed `eventType: "message"`, and `eventKind` (`user_request` or `room_event`) from the finalized inbound context. Optional `senderName`, `senderUsername`, and `senderAliases` values come from channel-authenticated metadata. They are textual hints only and never replace the opaque `senderId` or gain routing authority.
