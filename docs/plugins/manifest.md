...
- expected typed-hook registration contracts
...
| `expectedHooks`                      | No       | `string[]`                       | Canonical typed hook names that a successful full runtime registration must install. Status surfaces report these names without loading plugin code; runtime loading fails and rolls back partial registration when any declared hook is missing. |
