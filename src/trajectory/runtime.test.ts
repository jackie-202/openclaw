...
describe("trajectory runtime", () => {
  ...
  it("triggers immediate flushNow on critical events when the writer supports it", () => {
    const writes: string[] = [];
    ...
    const writerMock = {
      ...
      flushNow: async () => {
        flushNowCalls.push(writes.length);
      },
    ...
    const recorder = createTrajectoryRuntimeRecorder({
      sessionId: "session-1",
      sessionFile: "/tmp/session.jsonl",
      // Cast to QueuedFileWriter: the runtime accepts the legacy shape, the
      // recorder probes via isClosableWriter for the optional flushNow/close.
      writer: writerMock as unknown as Parameters<
        typeof createTrajectoryRuntimeRecorder
      >[0]["writer"],
    });

    recorder?.recordEvent("context.compiled", { foo: "bar" });
    recorder?.recordEvent("session.started", { runId: "r-1" });
    recorder?.recordEvent("session.ended", { status: "ok" });
    recorder?.recordEvent("tool.invoked", { name: "shell" });
    recorder?.recordEvent("custom.event", { critical: true });

    expect(writes).toHaveLength(5);
    // flushNow must fire for: session.started, session.ended, custom.event(critical=true)
    // but NOT for context.compiled or tool.invoked.
    expect(flushNowCalls).toHaveLength(3);
    // Reading flushNowCalls values: each captures writes.length at call time.
    expect(flushNowCalls).toEqual([2, 3, 5]);
    // No accidental flush() during recordEvent.
    expect(flushCalls).toBe(0);
  });

  it("does not call flushNow on critical events when writer is legacy (no flushNow)", () => {
    const writes: string[] = [];
    const recorder = createTrajectoryRuntimeRecorder({
      sessionId: "session-1",
      sessionFile: "/tmp/session.jsonl",
      writer: {
        ...
        flush: async () => undefined,
        // No flushNow / close → matches legacy QueuedFileWriter shape.
      },
    });

    // Must not throw despite session.started being a critical type.
    recorder?.recordEvent("session.started", { runId: "r-1" });
    expect(writes).toHaveLength(1);
  });
});
